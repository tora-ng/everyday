...
class Rectangle {
	...
	public static int getArrRec(Rectangle[] rec) { // rec에 대한 데이터 타입을 적어 줘야 한다.
		...
		return sum; //이 getArrRec 함수가 아래 class의 밖에 있기 때문에 아래 int area에서 getArrRec(rec)만 하면 오류가 난다. 그래서 이렇게 바깥쪽에 위치하게 되면 Rectangle.getArrRec(rec) 이렇게 쓰거나
					//아래 class에 속하도록 하고 getArrRec(rec) 이렇게 써 주는 방법이 있다.
	}
}
